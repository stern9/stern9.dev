...
// In-memory limits. Fine for a single Node process (pm2 fork mode); they
// reset on restart, which is acceptable for a joke bot.
...
export function checkLimits(ip) {
  ...
  if (dayCount >= LIMITS.dailyTotal) return false;

  const { max, windowMs } = LIMITS.perVisitor;
